...
final class DungeonTreeBuilder {
    ...
    static void fixStraightLeavesBump(Map<Point, Set<Point>> adj, Set<Point> occupied, Set<Point> exclude) {
        ...
        while (changed && guard++ < cap) {
            ...
            for (Point leaf : new ArrayList<>(adj.keySet())) {
                ...
                Point b1 = new Point(parent.x() - dy, parent.y() + dx, parent.level());
                Point b2 = new Point(parent.x() + dy, parent.y() - dx, parent.level());
