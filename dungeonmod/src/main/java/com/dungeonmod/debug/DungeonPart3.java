...
/**
 * Génération/analyse de la troisième partie du donjon : grande zone donjon,
 * bibliothèque, shop, hub Centrale et préparation de l'étage P4.
 *
...
final class DungeonPart3 {
    ...
    private static void growSplitBranches(Point e, Map<Point, Set<Point>> adj,
        ...
        for (int b = 0; b < Math.min(2, ad.size()); b++) {
            int bDir = ad.get(b);
            Point bk = e.move(DungeonAlgo.DIR_OFFSET[bDir]);
    ...
    private static TreeResult generateTrunkTree(int targetMin, int targetMax,
        ...
        for (int t = 1; t < trunkTarget; t++) {
            // Limite géométrique : max DungeonAlgo.MAX_COLINEAR_RUN segments alignés dans l'adj
        ...
        while (adj.size() < targetSize) {
            ...
            for (Point node : adj.keySet()) {
                ...
                for (int[] d : DungeonAlgo.DIR_OFFSET) {
                    ...
                    // Limite géométrique globale : pas plus de DungeonAlgo.MAX_COLINEAR_RUN
    ...
    static Map<Point, String> analyzePart3(Map<Point, Set<Point>> adj, Point campExit,
        ...
        // Couloirs MJ : même règle d'espacement (distance >= DungeonAlgo.MONSTER_MIN_DIST), sinon la
