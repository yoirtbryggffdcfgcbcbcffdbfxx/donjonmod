...
final class DungeonPart3 {
    ...
    private static TreeResult generateTrunkTree(int targetMin, int targetMax, Point startPt, Set<Point> blocked, int maxI3, int maxI4, Random rng) {
        ...
        // (B) "pas de cul-droit" a la source, sur l'arbre P3 (BUMP, start protege).
        DungeonTreeBuilder.fixStraightLeavesBump(adj, occupied, Set.of(start));
    ...
    static Map<Point, RoomType> analyzePart3(Map<Point, Set<Point>> adj, Point campExit,
        ...
        for (int corridorLen : new int[]{4}) {
            ...
            // Empreinte generique de la salle (size.x x size.z) : in-bounds + libre.
            if (!DungeonRoomPlacement.isFootprintFree(adj, new Point(hx2, hz2), RoomType.CENTRALE.size, null)) { removeNodesClean(adj, corrNodes); continue; }
