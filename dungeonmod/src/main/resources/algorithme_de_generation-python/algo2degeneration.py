...
ROOM_STYLES = {
    ...
    'Prison':   ((153, 51, 153), "Pris"),   # Prison (toujours cul-de-sac)
...
# ==============================================================================
# 1. GÉNÉRATION DE LA STRUCTURE GÉOMÉTRIQUE
# ==============================================================================
...
# ==============================================================================
# 2. LABELLISATION ET GESTION DES RÈGLES DE JEU
# ==============================================================================
...
def analyze_and_label(start_pos, adj):
    ...
    # Identifier les culs-de-sac (degré 1). On exclut le départ.
    ...
    for leaf in leaves:
        ...
        if len(adj[parent]) == 2:
            ...
            # Si les deux directions sont identiques (colinéarité)
    ...
    # Classifier les pièces intermédiaires restantes (degré 2 et 3)
    ...
    # Validation de la contrainte : pas plus de 2 couloirs 'C' consécutifs
...
# ==============================================================================
# 3. CALCUL DE L'ORIENTATION (PORTES ET ROTATION MINECRAFT)
# ==============================================================================

def get_room_orientation(node, neighbors):
    """Calcule les directions ouvertes pour un nœud sous forme de chaîne (ex: NES, EW)."""
    dirs = []
    for nx, ny in neighbors:
        dx, dy = nx - node[0], ny - node[1]
        if dx == 1 and dy == 0:
            dirs.append('E')  # East (Est)
        elif dx == -1 and dy == 0:
            dirs.append('W')  # West (Ouest)
        elif dx == 0 and dy == 1:
            dirs.append('S')  # South (Sud)
        elif dx == 0 and dy == -1:
            dirs.append('N')  # North (Nord)
            
    # Tri dans un ordre standard de lecture (N, S, E, W)
    dirs.sort(key=lambda x: "NSEW".index(x))
    return "".join(dirs)

# ==============================================================================
# 4. GESTION DE LA VALIDATION GENERALE
# ==============================================================================

def generate_valid_dungeon():
    """Boucle de génération-rejet jusqu'à obtenir un tracé qui valide toutes les règles."""
    for _ in range(5000):
...
# ==============================================================================
# 5. CODE PRINCIPAL (MAIN ET RENDU PYGAME)
# ==============================================================================

def main():
    pygame.init()
    
    # Récupération de la taille de l'écran physique
    ...
    # Ajustement des tailles des polices selon la résolution d'écran
    font_size = max(10, int(TILE_SIZE * 0.30))
    font = pygame.font.SysFont("Arial", font_size, bold=True)
    
    small_font_size = max(8, int(TILE_SIZE * 0.22))
    ...
    while running:
        ...
        if dungeon:
            ...
            for node, label in labels.items():
                ...
                # Ligne 1 : Nom de la salle (ex: I2, C, Pris)
                text_surf = font.render(text, True, text_color)
                text_rect = text_surf.get_rect(center=(sx, sy - TILE_SIZE // 6))
                screen.blit(text_surf, text_rect)
                
                # Ligne 2 : Orientation / Portes ouvertes (ex: NW, EW, NES)
                orientation_str = get_room_orientation(node, adj[node])
                ori_surf = small_font.render(orientation_str, True, text_color)
                ori_rect = ori_surf.get_rect(center=(sx, sy + TILE_SIZE // 4))
                screen.blit(ori_surf, ori_rect)
        ...
        # Alignement horizontal de la légende
        desc_y = y_info + 40
